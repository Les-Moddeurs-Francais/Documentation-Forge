...
Une fois cela fait, je vous invite à construire votre test. Je testerai ici le hopper à des fins de démonstrations. J'ai donc juste posé un hopper, comme cela :

![Une template avec un hopper](/img/docs/gametest_template_hopper.png)

Une fois votre modèle (le nom de ce que vous venez de créer) de test fini, il vous suffit de faire un clic droit sur le bloc de structure (celui qui est violet) et de cliquer sur le bouton "SAVE" **en bas à droite** (pas l'autre).
...
Allez dans le dossier run à la racine de votre projet. Ensuite, allez dans saves puis sélectionnez le dossier qui correspond au nom du monde dans lequel vous avez créé votre modèle. Il vous suffit juste d'aller dans generated/minecraft/structures et vous devriez trouver un fichier qui a le même nom que la modèle que vous avez créée. Copiez ce fichier et collez-le dans le dossier de vos structures (dans le dossier resources : data/modid/structures). Lorsque je parlerai du fichier sélectionné, ce sera de celui que vous venez de coller.
...
Soit utiliser l'événement RegisterGameTestsEvent :
```java
@SubscribeEvent
...
#### Renommer la modèle
Maintenant que vous avez toutes ces informations, je vous invite à renommer votre modèle (le fichier sélectionné) ou à ajuster la ou les annotations qui sont devant votre méthode.
...
Dans la classe `GameTestHelper`, lorsqu'une instance de la classe `BlockPos` est demandée, pour poser un bloc par exemple, **elle doit être en coordonnées relatives**, c'est-à-dire en coordonnées par rapport à l'origine de la modèle (le structure block).
:::

Comme dit plus haut, les méthodes de la class `GameTestHelper` sont là pour vous aider à remplir votre GameTest.

Dans mon exemple j'utiliserai le modèle créée plus haut
```java
...
Une fois le ou les tests lancés, vous devriez voir les modèles être ajoutées au monde et les tests s'exécuter.

Si un test est un succès, un beacon vert apparaîtra proche de la modèle.

Si un test échoue, un beacon rouge apparaîtra proche de la modèle ainsi qu'un livre contenant les erreurs posé sur un pupitre. Vous aurez aussi un message dans le chat.

...
:::warning
La classe `LegacyUpgradeRecipeBuilder` est marqué comme déprécié et sera donc supprimée dans les versions supérieures (sûrement pour la 1.20).
:::

```java
LegacyUpgradeRecipeBuilder.smithing(Ingredient.of(Items.STONE_PICKAXE), Ingredient.of(Items.IRON_INGOT), RecipeCategory.TOOLS, Items.IRON_PICKAXE)

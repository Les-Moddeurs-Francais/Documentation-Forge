...
```java
public static final RegistryObject<Block> MON_BLOC = BLOCKS.register("mon_bloc", () -> new Block(BlockBehaviour.Properties.of().mapColor(MapColor.STONE)));
```
...
```json
{
  ...
  "random_sequence": "modid:blocks/mon_bloc"
}
```
...
- `random_sequence` définit un "identifiant" de séquence permettant d'avoir les mêmes "recompenses" lâchées dans un monde avec la même _seed_.
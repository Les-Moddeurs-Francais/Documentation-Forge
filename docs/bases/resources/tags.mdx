...
| `cherry_logs`                              | Liste des blocs qui sont des bûches de cerisier                                                                                                                               |
...
| `enchantment_power_provider`               | Liste des blocs qui alimentent la table d'enchantement                                                                                                                        |
| `enchantment_power_transmitter`            | Liste des blocs qui autorisés entre la table d'enchantement et ceux lui fournissant de l'énergie                                                                              |
...
| `maintains_farmland`                       | Liste des blocs qui peuvent être placés sur de la terre labourée, empêchant cette dernière de se transformer en terre lorsqu'elle n'est pas hydratée                          |
...
| `replaceable`                              | Liste des blocs qui sont remplaçables (référence uniquement)                                                                                                                  |
| `replaceable_by_trees`                     | Liste des blocs que les arbres peuvent remplacer lorsqu'ils grandissent                                                                                                       |
...
| `smelts_to_glass`                          | Liste des blocs qui peuvent être cuits pour obtenir du verre                                                                                                                  |
| `snaps_goat_horn`                          | Liste des blocs qui cassent une corne aux chèvres quand elles chargent dedans                                                                                                 |
| `sniffer_diggable_block`                   | Liste des blocs dans lesquels les renifleurs sont autorisés à creuser                                                                                                         |
| `sniffer_egg_hatch_boost`                  | Liste des blocs sur lesquels les œufs de renifleurs éclosent plus rapidement.                                                                                                 |
...
| `stone_buttons`                            | Liste des blocs qui sont des boutons en pierre                                                                                                                                |
...
| `sword_efficient`                          | Liste des blocs qui sont plus rapides à briser avec une épée                                                                                                                  |
| `tall_flowers`                             | Liste des blocs qui sont des fleurs hautes                                                                                                                                    |
| `terracotta`                               | Liste des blocs qui sont de la terre cuite                                                                                                                                    |
| `trail_ruins_replaceable`                  | Liste des blocs que les ruines de sentiers peuvent remplacer lorsque ces dernières sont générées                                                                              |
...
| `vibration_resonators`                     | Liste des blocs qui peuvent être utilisés pour faire résonner les vibrations lorsqu'ils sont placés à côté d'un capteur sculk                                                 |
...
<summary>Liste des tags concernant les types de dégât</summary>
    
| Identifiant                    | Description                                                                                                                                                            |
|--------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `always_hurts_ender_dragons`   | Liste des types de dégâts qui touchent l'Ender Dragon à coup sûr (même si ces derniers ne sont pas source d'un joueur)                                                 |
| `always_most_significant_fall` | Liste des types de dégâts qui sont des chutes tuant à coup sûr                                                                                                         |
| `always_triggers_silverfish`   | Liste des types de dégâts qui réveillent à coup sûr un poisson d'argent (_silverfish_) quand un autre subit des dégâts dun même type                                   |
| `avoids_guardian_thorns`       | Liste des types de dégâts qui ne tiennent pas des dégâts provoqués en retour par les épines d'un gardien (l'entité à la source du dégât ne sera pas blessée en retour) |
| `burns_armor_stands`           | Liste des types de dégâts qui brulent un porte-armure                                                                                                                  |
| `bypasses_armor`               | Liste des types de dégâts qui ne tiennent pas compte de l'armure                                                                                                       |
| `bypasses_cooldown`            | Liste des types de dégâts qui ne tiennent pas compte du _cooldown_ (qui rend la cible invincible un court instant après avoir subi un dégât)                           |
| `bypasses_effects`             | Liste des types de dégâts qui ne tiennent pas compte des effets de potions                                                                                             |
| `bypasses_enchantments`        | Liste des types de dégâts ui ne tiennent pas compte des enchantements                                                                                                  |
| `bypasses_invulnerability`     | Liste des types de dégâts qui ne tiennent pas compte de l'invulnérabilité                                                                                              |
| `bypasses_resistance`          | Liste des types de dégâts qui ne tiennent pas compte de la resistance                                                                                                  |
| `bypasses_shield`              | Liste des types de dégâts qui ne tiennent pas compte du bouclier                                                                                                       |
| `damages_helmet`               | Liste des types de dégâts qui endommagent le casque                                                                                                                    |
| `ignites_armor_stands`         | Liste des types de dégâts qui mettent en feu un porte-armure                                                                                                           |
| `is_drowning`                  | Liste des types de dégâts de noyade                                                                                                                                    |
| `is_explosion`                 | Liste des types de dégâts d'explosion                                                                                                                                  |
| `is_fall`                      | Liste des types de dégâts de chute                                                                                                                                     |
| `is_fire`                      | Liste des types de dégâts de feu                                                                                                                                       |
| `is_freezing`                  | Liste des types de dégâts de froid                                                                                                                                     |
| `is_lightning`                 | Liste des types de dégâts causés par la foudre                                                                                                                         |
| `is_projectile`                | Liste des types de dégâts causés par des projectiles                                                                                                                   |
| `no_anger`                     | Liste des types de dégâts qui ne sont pas sources de colère (_aggro_)                                                                                                  |
| `no_impact`                    | Liste des types de dégâts qui ne sont pas sources d'un impact                                                                                                          |
| `witch_resistant_to`           | Liste des types de dégâts auxquels les sorcières sont résistantes                                                                                                      |
| `wither_immune_to`             | Liste des types de dégâts auxquels les Wither sont immunisés                                                                                                           |
...
| `dismounts_underwater`       | Liste des entités qui forcent le pilote (entité chevauchant l'autre) à descendre quand cette dernière rentre dans l'eau |
| `fall_damage_immune`         | Liste des entités qui sont immunisées contre les dégâts de chute                                                        |
...
| `axes`                         | Liste de tous les types d'haches                                                                                            |
| `axolotl_tempt_items`          | Liste des items permettant d'attirer des axolotl                                                                            |
| `bamboo_blocks`                | Liste de tous les types de blocs de bambou                                                                                  |
...
| `breaks_decorated_pots`        | Liste de tous les types d'objets qui permettent de casser des vases décorés instantanément                                  |
| `buttons`                      | Liste de tous les types de boutons                                                                                          |
| `candles`                      | Liste de tous les types de bougies                                                                                          |
| `cherry_logs`                  | Liste de tous les types de bûche de cerisier                                                                                |
...
| `decorated_pot_ingredients`    | Liste de tous les types d'ingrédients pouvant être utilisés pour fabriquer des vases décorés                                |
| `decorated_pot_sherds`         | Liste de tous les types de tessons permettant de décorer des vases décorés                                                  |
...
| `hoes`                         | Liste des minerais de faux                                                                                                  |
...
| `noteblock_top_instruments`    | Liste des tous les types de blocs qui peuvent être posés au-dessus d'un bloc musical                                        |
| `oak_logs`                     | Liste de tous les types de troncs en chêne                                                                                  |
| `pickaxes`                     | Liste de tous les types de pioches                                                                                          |
...
| `shovels`                      | Liste de tous les types de pelles                                                                                           |
...
| `smelts_to_glass`              | Liste des blocs qui peuvent être cuits pour obtenir du verre                                                                |
| `sniffer_food`                 | Liste de tous les types de nourritures pour les renifleurs                                                                  |
...
| `stone_buttons`                | Liste de tous les types de boutons en pierre                                                                                |
...
| `villager_plantable_seeds`     | Liste de tous les types de graines que les villageois fermiers planteront lorsqu'elles seront placées dans leur inventaire  |
...
| `has_structure\trail_ruins`                 | Liste des biomes pouvant contenir des ruines de sentiers                                                             |
